#!/bin/bash
#SBATCH --job-name=pgr_train_sem
#SBATCH --output=/scratch/%u/pgr_3d/outputs/logs/train_sem_%j.out
#SBATCH --error=/scratch/%u/pgr_3d/outputs/logs/train_sem_%j.err
#SBATCH --nodes=1
#SBATCH --gpus=h100:1
#SBATCH -p gpu-large
...
# ---------------------------------------------------------------------------
# Train SemanticHead (Day 5)
# Prerequisite: Objaverse rendered to /scratch/s224696943/pgr_3d/data/objaverse_renders
# ---------------------------------------------------------------------------
...
python "${PGR_DIR}/src/train_readout.py" \
    ...
    --lr 1e-4 \
    --weight_decay 0.01 \
    --run_name "pgr_semantic_200_${DATE}" \
    --ckpt_dir "${CKPT_DIR}" \
    --wandb_mode online

echo "SemanticHead training complete."
